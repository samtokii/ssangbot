...
@tree.command(name="지출선택삭제", description="지출 내역 중 원하는 항목을 번호로 삭제합니다")
@app_commands.describe(번호="삭제할 지출 번호 (지출내역 명령어로 확인 가능)")
async def 지출선택삭제(interaction: discord.Interaction, 번호: int):
    user_id = str(interaction.user.id)
    내역 = 지출_목록.get(user_id, [])
    if 번호 < 1 or 번호 > len(내역):
        await interaction.response.send_message("❌ 유효하지 않은 번호입니다")
        return
    삭제된 = 지출_목록[user_id].pop(번호 - 1)
    저장()
    await interaction.response.send_message(f"🗑️ 선택한 지출 삭제됨: {삭제된[0]}원 ({삭제된[1]})")

@tree.command(name="지출전체삭제", description="지출 내역을 모두 삭제합니다")
...
@tree.command(name="잔소리전체삭제", description="특정 카테고리의 잔소리를 전부 삭제합니다")
@app_commands.describe(카테고리이름="전체 삭제할 잔소리 카테고리 이름")
async def 잔소리전체삭제(interaction: discord.Interaction, 카테고리이름: str):
    if 카테고리이름 not in 잔소리:
        await interaction.response.send_message(f"❌ '{카테고리이름}' 카테고리를 찾을 수 없습니다")
        return
    삭제개수 = len(잔소리[카테고리이름])
    잔소리[카테고리이름] = []
    저장()
    await interaction.response.send_message(f"🧹 '{카테고리이름}' 카테고리의 잔소리 {삭제개수}개가 모두 삭제되었습니다")
